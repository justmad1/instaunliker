...
# Max recursion depth for liked_medias in "fast" mode
MAX_RECURSION_DEPTH = 50
MAX_UNLIKES_FOR_FAST_MODE = 20
...
def unlike_liked_posts(client):
    ...
    for post in liked:
        ...
        try:
            ...
            count += 1
            print(
                TEXT["unliking"].format(
                    id=post.id, username=post.user.username if post.user else ""
                )
            )
            ...
            if count % block_limit == 0 and count < MAX_UNLIKES_PER_RUN:
                ...
                msg = TEXT["pause_msg"].format(
                    minutes=round(long_pause / 60, 1), count=count
                )
                print(msg)
...
def unlike_liked_posts_fast(client, recursion_depth=0, count=0):
    if recursion_depth > MAX_RECURSION_DEPTH:
        print(TEXT["max_recursion_depth_reached"])
        return

    send_telegram_message(TEXT["start_fast"])
    if recursion_depth:
        print(TEXT["fetching"])
    else:
        print(TEXT["no_posts_from_unfollowed_users_found"])

    followed_users = client.user_following(client.user_id)
    followed_ids = set(followed_users.keys())

    chunk = client.liked_medias(amount=20)
    filtered = [
        post for post in chunk if post.user and post.user.pk not in followed_ids
    ]
    ...
    for post in filtered:
        ...
        try:
            ...
            count += 1
            print(TEXT["unliking"].format(id=post.id, username=post.user.username))
    ...
    if count < MAX_UNLIKES_FOR_FAST_MODE:
        time.sleep(5)
        return unlike_liked_posts_fast(client, recursion_depth + 1, count)
...
if __name__ == "__main__":
    ...
    if UNLIKE_MODE == "4":
        unlike_liked_posts_fast(client)
    else:
